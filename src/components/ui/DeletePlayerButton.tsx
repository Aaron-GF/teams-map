...
export default function DeletePlayerButton({
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    const result = await deletePlayer(playerId, imageUrl);
    setIsDeleting(false);
  ...
  return (
    <button
      ...
      disabled={isDeleting}
      className="p-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all border border-red-100 shadow-sm disabled:opacity-50"
